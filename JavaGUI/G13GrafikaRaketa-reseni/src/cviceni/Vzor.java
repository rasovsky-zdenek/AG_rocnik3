...
public class Vzor extends javax.swing.JFrame {
    ...
    private Canvas canvas; //platno uz sice existuje jako objekt panelPlatno, ale podle hesla Divide and Conquer ziskame platno jako samostatnu tridu
    private Rocket rocket;
    //vytvor promenne left, right typu boolean
    boolean left,right; //promenne pro zjisteni, jestli je klavesa stale stistena
    /**
     * vytvoreni objektu
     */
    public Vzor() {
        ...
        canvas = Canvas.prepareCanvas(panelPlatno);//pomoci Singleton vytvorime paltno; vzdy bude existovat jedina instance
        ...
        rocket = new Rocket("src/graphics/raketa.png");
        //umisti raketu na dolu na stred
        rocket.rocketOnStart(canvas);//canvas je predavan, aby bylo mozne raketu vykreslt na stred
...
public void kresli(Graphics gr){
    //System.out.println("Kreslím!");
     ...
     //zamez, aby raketa nemohla opustit platno
        if(left){
           rocket.setX(rocket.getX()-getDeltaS());
           if(rocket.getX() < 0) rocket.setX(0); 
        }
        //jestli je right true, pak nastav posun rakety o hodnotu, kterou vrati metoda getDeltaS
        //zamez, aby raketa nemohla opustit platno
        if(right){
           rocket.setX(rocket.getX()+getDeltaS());
           if((rocket.getX() + rocket.getRocketWidth()) > canvas.getWidth()) {
               rocket.setX(canvas.getWidth() - rocket.getRocketWidth()); 
           }
        }
    ...
    private int getDeltaS() {
        ...
        float deltaSf;
        deltaSf = rocket.getSpeed() * canvas.getTimerDelay()/1000f;
        //System.out.println(deltaSf);
        return Math.round(deltaSf); 
        //System.out.println(Math.round(deltaSf)); 
    }
}
